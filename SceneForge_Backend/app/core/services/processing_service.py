...
class ProcessingService:
    ...
    async def process_scene(self, 
        ...
        Pipeline: Image → Depth Map → Point Cloud → Mesh → Export
        """
        try:
            # Convert output_path to Path object early to avoid string division errors downstream
            ...
            # Load input file
            await update_callback(job_id, 10, "Loading and analyzing input...")
            
            # Convert to absolute path if relative
            if not os.path.isabs(input_path):
                input_path = os.path.abspath(input_path)
            
            # Verify file exists after path conversion
            if not os.path.exists(input_path):
                raise Exception(f"Input file not found after path conversion: {input_path}")
            
            if file_ext in ['.mp4', '.avi', '.mov']:
                frames = self._extract_frames(input_path)
                input_type = 'video'
            elif file_ext in ['.glb', '.gltf', '.obj']:
                # Handle 3D model files - optimize and export
                await update_callback(job_id, 50, "Loading 3D model...")
                result = await self._process_3d_model(input_path, output_path, job_id, update_callback)
                return result
            else:
                # Try PIL first, then imageio (for AVIF), then fallback to OpenCV
                frames = []
                try:
                    img = Image.open(input_path)
                    # Force load the image data so PIL has it in memory
                    img.load()
                    frames = [img]
                    logger.info(f"Loaded image via PIL")
                except Exception as pil_error:
                    logger.warning(f"PIL failed: {str(pil_error)}")
